...
class AI_Core(QObject):
    ...
    async def send_realtime(self):
        """ [MODIFIED] Uses the new send_realtime_input method """
        while self.is_running:
            msg = await self.out_queue_gemini.get()
            if not self.is_running: break
            # Use send_realtime_input for streaming media to avoid deprecation warning
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # [FIXED] Changed QTextEdit to QLabel to fix the AttributeError
        self.sources_display = QLabel()
        self.sources_display.setWordWrap(True)
        self.sources_display.setAlignment(Qt.AlignTop)
        self.sources_display.setOpenExternalLinks(True) # This now works on a QLabel
    ...
    def update_search_results(self, urls):
        ...
        # If no new URLs are provided, do nothing, leaving the previous links visible.
        if not urls:
            return
