...
class AI_Core(QObject):
    ...
    async def receive_text(self):
        """ [MODIFIED] Receives text from Gemini, handles tool usage (search & code), and emits signals. """
        while self.is_running:
            try:
                turn_urls = set()
                        ...
                        if model_turn:
                            for part in model_turn.parts:
                                if part.executable_code is not None:
                                    # This part contains a tool call that the model wants to run.
                                    # The SDK automatically determines if it's a search or code.
                                    code = part.executable_code.code
                                    
                                    # Heuristic to decide if it's code or a search query
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.tool_activity_title.setStyleSheet("font-size: 16px; font-weight: bold; color: #a9b7c6; padding: 5px; border: none;")
        ...
        self.tool_activity_display.setTextInteractionFlags(Qt.TextSelectableByMouse) 
        ...
        """)
        self.left_layout.addWidget(self.tool_activity_display)
    ...
    def update_search_results(self, urls):
        ...
        if not urls:
            if not self.tool_activity_display.text().startswith('<pre>'):
                 self.tool_activity_display.clear()
                 self.tool_activity_title.setText("Tool Activity")
            return

        self.tool_activity_display.clear()
        self.tool_activity_title.setText("Search Sources Used")
    ...
    def display_executed_code(self, code):
        """Displays the Python code being executed by the AI in the left panel."""
        if not code:
            if "href" not in self.tool_activity_display.text():
                self.tool_activity_display.clear()
                self.tool_activity_title.setText("Tool Activity")
            return
        ...
        html_content = f'<pre style="white-space: pre-wrap; word-wrap: break-word; font-family: Consolas, monaco, monospace; color: #A9B7C6; font-size: 12px;">{escaped_code}</pre>'
        self.tool_activity_display.setText(html_content)
